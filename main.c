...
t_ent *insert_ent(t_ent *root, t_ent *z) {
    t_ent *y = NULL;
    t_ent *x = root;
    while (x != NULL) {
        y = x;
...
t_ent *is_in_ent(t_ent *x, char *id_ent) {
    if (x == NULL || strcmp(id_ent, x->id_ent) == 0) {
        return x;
    }
...
void insert_orig(t_rel *relation, t_dest *dest, char *id_orig) {
    ...
    dest->orig_root = insert_ent(dest->orig_root, new_orig);
    dest->counter++;
...
void add_ent(char *id_ent) {
    if (is_in_ent(ent_root, id_ent) == NULL) {
        t_ent *new_ent = (t_ent *) malloc(sizeof(t_ent));
        strcpy(new_ent->id_ent, id_ent);
        ent_root = insert_ent(ent_root, new_ent);
    }
}
...
void add_rel(char *id_orig, char *id_dest, char *id_rel) {
    if (is_in_ent(ent_root, id_orig) != NULL && is_in_ent(ent_root, id_dest) != NULL) {
        ...
        if (next_rel == NULL && last_rel == NULL) {
            ...
            if (strcmp(last_rel->id_rel, id_rel) == 0) {
                ...
                if (dest == NULL) {
                    ...
                    if (is_in_ent(dest->orig_root, id_orig) == NULL) {
