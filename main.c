...
t_ent *insert_orig(t_rel_list *relation ,t_rel_node *dest , char *id_orig, t_ent *prev, t_ent *next){
    ...
    while (dest->prev != NULL && (dest->counter > dest->prev->counter || ( dest->counter == dest->prev->counter && strcmp(dest->id_dest, dest->prev->id_dest)<0 ))){
...
void del_ent(char *id_ent){
    ...
    if(ent_to_del != NULL){
        ...
        do{
            t_rel_node *dest_to_del = is_in_dest(id_ent, rel_pnt->node_list);
            if(dest_to_del != NULL){
                if(dest_to_del->prev != NULL && dest_to_del->next != NULL){
                    dest_to_del->prev->next = dest_to_del->next;
                    dest_to_del->next->prev = dest_to_del->prev;
                } else if(dest_to_del->prev == NULL && dest_to_del->next != NULL){
                    dest_to_del->next->prev = NULL;
                    rel_pnt->node_list = dest_to_del->next;
                } else if(dest_to_del->prev != NULL && dest_to_del->next == NULL){
                    dest_to_del->prev->next = NULL;
                    rel_pnt->last_node = dest_to_del->prev;
                } else if(dest_to_del->prev == NULL && dest_to_del->next == NULL){
    ...
    if(is_in_ent(id_orig) != NULL && is_in_ent(id_dest) != NULL){
        ...
        if(next_rel == NULL && last_rel_node == NULL){
                    ...
                    if(next_orig == NULL && dest->last_orig == NULL){
                        ...
                        if(next_orig->prev != NULL){
                            if(strcmp(next_orig->prev->id_ent, id_orig)!=0){
                                ...
                                insert_orig(last_rel_node, dest, id_dest, next_orig->prev, next_orig);
                            }
                        } else{
            ...
            if(next_rel->prev == NULL){
                ...
                t_rel_node *new_dest = insert_dest(new_rel, id_dest);
                insert_orig(new_rel, new_dest, id_orig, NULL, NULL);
            } else if(strcmp(next_rel->prev->id_rel, id_rel)==0){
                ...
                if(dest == NULL){
                    ...
                    if(next_orig == NULL && dest->last_orig == NULL){
                        ...
                        if(next_orig->prev != NULL){
                            if(strcmp(next_orig->prev->id_ent, id_orig)!=0){
                                //ORIGINI: aggiungere in mezzo
                                insert_orig(next_rel->prev, dest, id_dest, next_orig->prev, next_orig);
                            }
                        } else{
                            //ORIGINI: aggiungere in testa
                            insert_orig(next_rel->prev, dest, id_orig, NULL, next_orig);
                        }
                    }
                }
...
void del_rel(char *id_orig, char *id_dest, char *id_rel){
    ...
    if(relation != NULL){
        ...
        if(dest != NULL){
            ...
            if(orig != NULL){
                if(orig->next != NULL && orig->prev != NULL){
                    orig->prev->next = orig->next;
                    orig->next->prev = orig->prev;
                } else if(orig->next == NULL && orig->prev != NULL){
                    orig->prev->next = NULL;
                ...
                while (dest->next != NULL && (dest->counter < dest->next->counter || ( dest->counter == dest->next->counter && strcmp(dest->id_dest, dest->next->id_dest)>0))){
