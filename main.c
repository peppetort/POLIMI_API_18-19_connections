...
t_rel_node *dest_get_next(char *id_dest, t_rel_node *relation){
...
t_rel_node *insert_dest(t_rel_list *relation, char *id_dest, t_rel_node *prev, t_rel_node *next) {
    ...
    if(prev == NULL && next == NULL){
        ...
        new_dest->prev = NULL;
        new_dest->next = relation->node_list;
        relation->node_list->prev = new_dest;
        relation->node_list = new_dest;
    } else if(prev != NULL && next == NULL){
...
t_ent *insert_orig(t_rel_node *dest, char *id_orig, t_ent *prev, t_ent *next) {
...
void delete_orig(t_rel_node *dest, t_ent *orig_to_del) {
...
void add_rel(char *id_orig, char *id_dest, char *id_rel) {
    if (is_in_ent(id_orig) != NULL && is_in_ent(id_dest) != NULL) {
        ...
        if (next_rel == NULL && last_rel_node == NULL) {
            ...
            t_rel_node *new_dest = insert_dest(new_rel, id_dest, NULL, NULL);
            insert_orig(new_dest, id_orig, NULL, NULL);
            ...
            if (strcmp(last_rel_node->id_rel, id_rel) == 0) {
                ...
                t_rel_node *next_dest = dest_get_next(id_dest, last_rel_node->node_list);
                if(next_dest == NULL && last_rel_node->last_node == NULL){
                    //DESTINAZIONE: lista vuota
                    t_rel_node *new_dest = insert_dest(last_rel_node, id_dest, NULL, NULL);
                    insert_orig(new_dest, id_orig, NULL, NULL);
                }else if(next_dest == NULL && last_rel_node->last_node != NULL){
                    if(strcmp(last_rel_node->last_node->id_dest, id_dest)==0){
                        //DESTINAZIONE: presente in coda -> controllare origini
                        t_ent *next_orig = ent_get_next(id_orig, last_rel_node->last_node->orig_list);
                        if (next_orig == NULL && last_rel_node->last_node->last_orig == NULL) {
                            //ORIGINI: lista vuota
                            insert_orig(last_rel_node->last_node, id_orig, NULL, NULL);
                        } else if (next_orig == NULL && last_rel_node->last_node->last_orig != NULL) {
                            if (strcmp(last_rel_node->last_node->last_orig->id_ent, id_orig) != 0) {
                                //ORIGINI: aggiungere in coda
                                insert_orig(last_rel_node->last_node, id_orig, last_rel_node->last_node->last_orig, NULL);
                            }
                        } else if (next_orig != NULL) {
                            if (next_orig->prev != NULL) {
                                if (strcmp(next_orig->prev->id_ent, id_orig) != 0) {
                                    //ORIGINI: aggiungere in mezzo
                                    insert_orig(last_rel_node->last_node, id_orig, next_orig->prev,
                                                next_orig);
                                }
                            } else {
                                //ORIGINI: aggiungere in testa
                                insert_orig(last_rel_node->last_node, id_orig, NULL,
                                            last_rel_node->last_node->orig_list);
                            }
                        }
                    } else{
                        //DESTINAZIONE: aggiungere in coda
                        t_rel_node *new_dest = insert_dest(last_rel_node, id_dest, last_rel_node->last_node, NULL);
                        insert_orig(new_dest, id_orig, NULL, NULL);
                    }
                } else if(next_dest != NULL){
                    if(next_dest->prev != NULL){
                        if(strcmp(next_dest->prev->id_dest, id_dest)==0){
                            //DESTINAZIONE: controlare le origini
                            t_ent *next_orig = ent_get_next(id_orig, next_dest->prev->orig_list);
                            if (next_orig == NULL && next_dest->prev->last_orig == NULL) {
                                //ORIGINI: lista vuota
                                insert_orig(next_dest->prev, id_orig, NULL, NULL);
                            } else if (next_orig == NULL && next_dest->prev->last_orig != NULL) {
                                if (strcmp(next_dest->prev->last_orig->id_ent, id_orig) != 0) {
                                    //ORIGINI: aggiungere in coda
                                    insert_orig(next_dest->prev, id_orig, next_dest->prev->last_orig, NULL);
                                }
                            } else if (next_orig != NULL) {
                                if (next_orig->prev != NULL) {
                                    if (strcmp(next_orig->prev->id_ent, id_orig) != 0) {
                                        //ORIGINI: aggiungere in mezzo
                                        insert_orig(next_dest->prev, id_orig, next_orig->prev,
                                                    next_orig);
                                    }
                                } else {
                                    //ORIGINI: aggiungere in testa
                                    insert_orig(next_dest->prev, id_orig, NULL,
                                                next_dest->prev->orig_list);
                                }
                            }
                        } else{
                            //DESTINAZIONE: aggiungere in mezzo
                            t_rel_node *new_dest = insert_dest(last_rel_node, id_dest, next_dest->prev, next_dest);
                            insert_orig(new_dest, id_orig, NULL, NULL);
                        }
                    } else{
                        //DESTINAZIONE: aggiunegere in testa
                        t_rel_node *new_dest = insert_dest(last_rel_node, id_dest, NULL, last_rel_node->node_list);
                        insert_orig(new_dest, id_orig, NULL, NULL);
                    }
                }
            } else {
            ...
            if (next_rel->prev == NULL) {
                ...
                if (strcmp(next_rel->prev->id_rel, id_rel) == 0) {
                    ...
                    t_rel_node *next_dest = dest_get_next(id_dest, next_rel->prev->node_list);
                    if(next_dest == NULL && next_rel->prev->last_node == NULL){
                        //DESTINAZIONE: lista vuota
                        t_rel_node *new_dest = insert_dest(next_rel->prev, id_dest, NULL, NULL);
                        insert_orig(new_dest, id_orig, NULL, NULL);
                    }else if(next_dest == NULL && next_rel->prev->last_node != NULL){
                        if(strcmp(next_rel->prev->last_node->id_dest, id_dest)==0){
                            //DESTINAZIONE: presente in coda -> controllare origini
                            t_ent *next_orig = ent_get_next(id_orig, next_rel->prev->last_node->orig_list);
                            if (next_orig == NULL && next_rel->prev->last_node->last_orig == NULL) {
                                //ORIGINI: lista vuota
                                insert_orig(next_rel->prev->last_node, id_orig, NULL, NULL);
                            } else if (next_orig == NULL && next_rel->prev->last_node->last_orig != NULL) {
                                if (strcmp(next_rel->prev->last_node->last_orig->id_ent, id_orig) != 0) {
                                    //ORIGINI: aggiungere in coda
                                    insert_orig(next_rel->prev->last_node, id_orig, next_rel->prev->last_node->last_orig, NULL);
                                }
                            } else if (next_orig != NULL) {
                                if (next_orig->prev != NULL) {
                                    if (strcmp(next_orig->prev->id_ent, id_orig) != 0) {
                                        //ORIGINI: aggiungere in mezzo
                                        insert_orig(next_rel->prev->last_node, id_orig, next_orig->prev,
                                                    next_orig);
                                    }
                                } else {
                                    //ORIGINI: aggiungere in testa
                                    insert_orig(next_rel->prev->last_node, id_orig, NULL,
                                                next_rel->prev->last_node->orig_list);
                                }
                            }
                        } else{
                            //DESTINAZIONE: aggiungere in coda
                            t_rel_node *new_dest = insert_dest(next_rel->prev, id_dest, next_rel->prev->last_node, NULL);
                            insert_orig(new_dest, id_orig, NULL, NULL);
                        }
                    } else if(next_dest != NULL){
                        if(next_dest->prev != NULL){
                            if(strcmp(next_dest->prev->id_dest, id_dest)==0){
                                ...
                                //DESTINAZIONE: aggiungere in mezzo
                                t_rel_node *new_dest = insert_dest(next_rel->prev, id_dest, next_dest->prev, next_dest);
                                insert_orig(new_dest, id_orig, NULL, NULL);
                            }
                        } else{
                            //DESTINAZIONE: aggiunegere in testa
                            t_rel_node *new_dest = insert_dest(next_rel->prev, id_dest, NULL, next_rel->prev->node_list);
                            insert_orig(new_dest, id_orig, NULL, NULL);
                        }
                    }
                } else {
